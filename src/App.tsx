...
function InviteModal({ workspaceId, onClose, onDone }: { workspaceId: string; onClose: () => void; onDone: (emails: string[], role: 'editor' | 'viewer') => void }) {
  ...
  if (links.length) return <Modal title="Davet bağlantıları hazır" subtitle="Her bağlantıyı ilgili kişiye özel olarak gönderin." onClose={() => onDone(emails, role)}><div className="invite-results">{links.map(item => <div className="invite-link-row" key={item.email}><div><strong>{item.email}</strong><span>Tek kullanımlık · 1 saat geçerli</span></div><button type="button" className="secondary-button" onClick={async () => { await navigator.clipboard.writeText(item.actionLink); setCopied(item.email) }}>{copied === item.email ? <><Check size={15}/> Kopyalandı</> : 'Bağlantıyı kopyala'}</button></div>)}</div><div className="inline-note"><ShieldCheck /><p>Bağlantıları WhatsApp, Discord veya istediğiniz kanaldan paylaşabilirsiniz. E-posta gönderim kotası kullanılmaz.</p></div><div className="modal-actions"><button type="button" className="primary-button" onClick={() => onDone(emails, role)}>Bitti</button></div></Modal>
