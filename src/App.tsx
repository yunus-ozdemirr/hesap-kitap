...
function TransactionModal({ ledger, initial, onClose, onSave }: { ledger: LedgerState; initial?: Transaction | null; onClose: () => void; onSave: (input: TransactionInput, file?: File) => Promise<void> }) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (amountMinor <= 0) { setError('Geçerli ve sıfırdan büyük bir tutar girin.'); return }
    if (!form.description.trim()) { setError('Açıklama zorunludur.'); return }
    setSaving(true)
    try { await onSave({ ...form, amount_minor: amountMinor, project_id: form.project_id || null, category: form.category || null, member_id: form.payment_source === 'member' ? form.member_id || ledger.members[0]?.user_id : null }, file) }
