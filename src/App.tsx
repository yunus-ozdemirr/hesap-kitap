...
function TransactionModal({ ledger, onClose, onSave }: { ledger: LedgerState; onClose: () => void; onSave: (input: TransactionInput, file?: File) => Promise<void> }) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (file && (file.size > 10 * 1024 * 1024 || !['application/pdf','image/jpeg','image/png'].includes(file.type))) { setError('Belge PDF, JPG veya PNG olmalı ve 10 MB’ı geçmemeli.'); return }
...
function InviteModal({ workspaceId, onClose, onDone }: { workspaceId: string; onClose: () => void; onDone: (emails: string[], role: 'editor' | 'viewer') => void }) {
  ...
  return <Modal title="Ekibini davet et" subtitle="E-postaları topluca ekle; herkes kendi güvenli bağlantısını alsın." onClose={onClose}><form className="form-stack" onSubmit={async e => { e.preventDefault(); setError(''); if (!emails.length) { setError('En az bir e-posta yazın.'); return } if (invalidEmails.length) { setError(`Geçersiz e-posta: ${invalidEmails.join(', ')}`); return } if (emails.length > 50) { setError('Tek seferde en fazla 50 davet gönderilebilir; kalanları yeni bir grupta gönderin.'); return } setSending(true); try { if (isConfigured) await createInvites(workspaceId, emails, role); onDone(emails, role) } catch (err) { setError(err instanceof Error ? err.message : 'Davetler gönderilemedi') } finally { setSending(false) } }}><label className="field">E-posta adresleri<textarea required rows={6} value={rawEmails} onChange={e => setRawEmails(e.target.value)} placeholder={'ayse@example.com\nmehmet@example.com\nece@example.com'} /><small>Virgül, boşluk veya yeni satırla ayırın · {emails.length}/50 adres</small></label><label className="field">Bu grubun rolü<select value={role} onChange={e => setRole(e.target.value as 'editor' | 'viewer')}><option value="viewer">Görüntüleyici — yalnızca kasa ve giderleri görür</option><option value="editor">Düzenleyici — gider ve proje ekleyebilir</option></select></label><div className="inline-note"><ShieldCheck /><p>Davetler uygulama tarafından gönderilir. Toplam üye sınırı yoktur; güvenlik ve e-posta hız sınırı için her gönderim en fazla 50 kişidir.</p></div>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="text-button" onClick={onClose}>Vazgeç</button><button className="primary-button" disabled={sending}>{sending ? 'Davetler gönderiliyor…' : `${emails.length || ''} davet gönder`}</button></div></form></Modal>
}
